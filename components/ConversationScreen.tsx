import { AttachFile, InsertEmoticon, InsertEmoticonOutlined, Mic, MoreVert, Send } from "@mui/icons-material"
...
const EndOfMessageForAuthScroll = styled.div`
  margin-bottom: 30px;
`
...
const ConversationScreen = ({ conversation, messages }: {conversation: Conversation, messages: IMessage[]}) => {
  ...
  const showMessages = () => {
    // if front-end is loading messages behind the scenes, display message retrieved from Next SSR (passed down from [id].tsx)
    if (messagesLoading) {
    ...
    // if front-end has finished loading messages, so now we have messageSnapshot
    if (messagesSnapshot) {
  ...
  const sendMessage: MouseEventHandler<HTMLButtonElement> = event => {
    event.preventDefault()
    if (!newMessage) {
      return
    }
